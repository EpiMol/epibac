...
# Función para ejecutar comandos como el usuario real sin prompts de sudo
run_as_real_user() {
    local cmd="$1"
    if [[ "$REAL_USER" != "root" ]]; then
        # Usar 'su' en lugar de 'sudo -u' para evitar prompts de contraseña
        su - "$REAL_USER" -c "$cmd"
    else
        # Si somos root real, ejecutar directamente
        bash -c "$cmd"
    fi
}

# Función para generar el contenido de configuración EPIBAC
generate_epibac_config() {
    local config_content=""
    
    # Agregar configuración de conda si está instalada
    if [[ "$CONDA_INSTALLED" == true ]]; then
        config_content+="# Conda configuration"$'\n'
        config_content+="export PATH=\"$REAL_USER_HOME/miniconda3/bin:\$PATH\""$'\n'
        config_content+=""$'\n'
        config_content+="# Conda initialization"$'\n'
        config_content+="__conda_setup=\"\$('$REAL_USER_HOME/miniconda3/bin/conda' 'shell.bash' 'hook' 2> /dev/null)\""$'\n'
        config_content+="if [ \$? -eq 0 ]; then"$'\n'
        config_content+="    eval \"\$__conda_setup\""$'\n'
        config_content+="else"$'\n'
        config_content+="    if [ -f \"$REAL_USER_HOME/miniconda3/etc/profile.d/conda.sh\" ]; then"$'\n'
        config_content+="        . \"$REAL_USER_HOME/miniconda3/etc/profile.d/conda.sh\""$'\n'
        config_content+="    else"$'\n'
        config_content+="        export PATH=\"$REAL_USER_HOME/miniconda3/bin:\$PATH\""$'\n'
        config_content+="    fi"$'\n'
        config_content+="fi"$'\n'
        config_content+="unset __conda_setup"$'\n'
        config_content+=""$'\n'
    fi
    
    # Agregar configuración de Go si está instalada
    if [[ "$GO_INSTALLED" == true ]]; then
        config_content+="# Go configuration"$'\n'
        config_content+="export GOPATH=\$HOME/go"$'\n'
        config_content+="export PATH=\"/usr/local/go/bin:\$GOPATH/bin:\$PATH\""$'\n'
        config_content+=""$'\n'
    fi
    
    # Agregar configuración de Singularity si está instalada
    if [[ "$SINGULARITY_INSTALLED" == true ]]; then
        config_content+="# Singularity configuration"$'\n'
        config_content+="export PATH=\"/usr/local/bin:\$PATH\""$'\n'
        config_content+=""$'\n'
    fi
    
    # Agregar alias de snake_env si se solicitó
    if [[ "$SNAKE_ENV_ALIAS_REQUESTED" == true && -n "$SNAKE_ENV_PATH" ]]; then
        config_content+="# Snake environment alias"$'\n'
        config_content+="alias snake_env='source $SNAKE_ENV_PATH'"$'\n'
        config_content+=""$'\n'
    fi
...
# Función para actualizar la configuración de shell con bloques EPIBAC_CONFIG
update_epibac_shell_config() {
    local shell_file="$1"
    local config_content
    
    if [[ ! -f "$shell_file" ]]; then
        format_message "INFO" "Creating $shell_file" "$INFO"
        run_as_real_user "touch '$shell_file'"
    fi
    
    # Generar el contenido de configuración
    config_content=$(generate_epibac_config)
    
    if [[ -z "$config_content" ]]; then
        format_message "WARNING" "No configuration to add to $shell_file" "$WARNING"
        return 0
    fi
    
    # Verificar si ya existe un bloque EPIBAC_CONFIG
    if run_as_real_user "grep -q '$EPIBAC_TAG_START' '$shell_file'"; then
        format_message "INFO" "Updating existing EPIBAC configuration block in $shell_file" "$INFO"
        
        # Crear archivo temporal con el contenido actualizado
        local temp_file="/tmp/shell_config_update_$$"
        
        # Extraer contenido antes del bloque EPIBAC_CONFIG
        run_as_real_user "sed '/$EPIBAC_TAG_START/,/$EPIBAC_TAG_END/d' '$shell_file'" > "$temp_file"
        
        # Agregar el nuevo bloque EPIBAC_CONFIG
        {
            echo "$EPIBAC_TAG_START"
            echo "$config_content"
            echo "$EPIBAC_TAG_END"
        } >> "$temp_file"
        
        # Reemplazar el archivo original
        run_as_real_user "cp '$temp_file' '$shell_file'"
        rm -f "$temp_file"
    else
        format_message "INFO" "Adding new EPIBAC configuration block to $shell_file" "$INFO"
        
        # Agregar el bloque al final del archivo
        run_as_real_user "cat >> '$shell_file'" << EOF
...
EOF
    ...
    format_message "OK" "Successfully updated $shell_file" "$OK"
}

# Función para limpiar la configuración EPIBAC de los archivos de shell
clean_epibac_shell_config() {
    local shell_files=("$REAL_USER_HOME/.bashrc" "$REAL_USER_HOME/.zshrc")
    
    for shell_file in "${shell_files[@]}"; do
        if [[ -f "$shell_file" ]]; then
            # Verificar si existe un bloque EPIBAC_CONFIG
            if run_as_real_user "grep -q '$EPIBAC_TAG_START' '$shell_file'"; then
                format_message "INFO" "Removing EPIBAC configuration block from $shell_file" "$INFO"
                
                # Crear archivo temporal sin el bloque EPIBAC_CONFIG
                local temp_file="/tmp/shell_config_clean_$$"
                
                # Extraer contenido sin el bloque EPIBAC_CONFIG
                run_as_real_user "sed '/$EPIBAC_TAG_START/,/$EPIBAC_TAG_END/d' '$shell_file'" > "$temp_file"
                ...
                format_message "OK" "Removed EPIBAC configuration from $shell_file" "$OK"
            else
                format_message "INFO" "No EPIBAC configuration found in $shell_file" "$INFO"
            fi
        else
            format_message "INFO" "Shell file $shell_file does not exist" "$INFO"
        fi
    done
}

# Funciones auxiliares para manejo de bloques de configuración
has_config_block() {
    local file="$1"
    local start_tag="$2"
    grep -q "$start_tag" "$file" 2>/dev/null
}
...
remove_config_block() {
    ...
    local start_tag="$2"
    local end_tag="$3"
    local temp_file="/tmp/config_remove_$$"
    
    # Crear backup
    cp "$file" "$file.bak_$(date +%Y%m%d%H%M%S)"
    
    # Eliminar el bloque de configuración
    sed "/$start_tag/,/$end_tag/d" "$file" > "$temp_file"
    mv "$temp_file" "$file"
    
    format_message "INFO" "Removed configuration block from $(basename "$file")" "$INFO"
}
...
add_config_block() {
    ...
    format_message "INFO" "Added configuration block to $(basename "$file")" "$INFO"
}
...
# Funciones duplicadas eliminadas - se mantienen las funciones principales al inicio del script
...
# Actualizar configuración de shell para .bashrc si existe
if [[ -f "$REAL_USER_HOME/.bashrc" ]]; then
    update_epibac_shell_config "$REAL_USER_HOME/.bashrc"
fi

# Actualizar configuración de shell para .zshrc si existe
if [[ -f "$REAL_USER_HOME/.zshrc" ]]; then
    update_epibac_shell_config "$REAL_USER_HOME/.zshrc"
fi
